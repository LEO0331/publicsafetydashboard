...
- `.github/workflows/pages.yml`: publishes a static GitHub Pages landing page when repository Pages is set to `GitHub Actions`.

GitHub Pages cannot run the full application because this project depends on Next.js API routes, SQLite, Python PDF import scripts, and server-side admin endpoints. Use GitHub Pages only as a static public project page. Use the GHCR Docker image for the working dashboard.

## GitHub Pages Static Preview

To make something appear when Repository Settings → Pages → Source is set to `GitHub Actions`:

1. Push `.github/workflows/pages.yml` to `main`.
2. Go to repository Settings → Pages.
3. Set Source to `GitHub Actions`.
4. Open Actions → `Deploy GitHub Pages`.
5. Run it manually or wait for the next push to `main`.
6. Open the Pages URL shown in the workflow environment.

The static artifact is generated by:

```bash
npm run pages:build
```

It writes `github-pages-dist/index.html`, which is uploaded by `actions/upload-pages-artifact`.
