    ...
    <main className="min-h-screen text-[var(--ink)]" data-testid="admin-page">
      <header className="border-b border-[var(--line)] bg-[var(--paper-strong)]">
        <div className="mx-auto flex max-w-6xl flex-col gap-4 px-4 py-7 md:flex-row md:items-end md:justify-between md:px-8">
          <div>
            ...
            <h1 className="font-display text-3xl font-semibold md:text-4xl">匯入管理</h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--muted)]">爬取政府公告頁、匯入 PDF 網址或上傳本機 PDF。所有來源與解析結果會寫入匯入紀錄。</p>
          </div>
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 md:px-8">
        <label className="ledger-panel flex flex-col gap-2 p-4 text-sm font-medium md:flex-row md:items-center">
          ...
          <input value={token} onChange={(event) => setToken(event.target.value)} className="focus-ring w-full border border-[var(--line)] bg-white p-2 font-normal" type="password" placeholder="請輸入 ADMIN_TOKEN" data-testid="admin-token" />
        </label>

        <div className="mt-5 grid gap-4 md:grid-cols-3">
          <form onSubmit={crawl} className="ledger-panel p-4">
            <div className="flex items-center justify-between">
              <h2 className="font-medium">政府頁面爬取</h2>
              <Globe2 className="text-[var(--civic-green)]" size={18} />
            </div>
            <p className="mt-3 text-sm leading-6 text-[var(--muted)]">依分頁循序抓取 PDF 連結，保留來源網址與公告標題。</p>
            <input name="maxPages" className="focus-ring mt-4 w-full border border-[var(--line)] bg-white p-2 text-sm" placeholder="最多頁數，可留空" />
          ...
          <form onSubmit={importUrl} className="ledger-panel p-4">
            <div className="flex items-center justify-between">
              <h2 className="font-medium">PDF 網址匯入</h2>
              <Upload className="text-[var(--signal-red)]" size={18} />
            </div>
            <p className="mt-3 text-sm leading-6 text-[var(--muted)]">匯入單一公告 PDF，下載後以內容雜湊去重。</p>
            <input name="title" className="focus-ring mt-4 w-full border border-[var(--line)] bg-white p-2 text-sm" placeholder="公告標題" />
            <input name="url" className="focus-ring mt-2 w-full border border-[var(--line)] bg-white p-2 text-sm" placeholder="PDF 網址" data-testid="pdf-url-input" />
          ...
          <form onSubmit={importFile} className="ledger-panel p-4">
            <div className="flex items-center justify-between">
              <h2 className="font-medium">本機 PDF 匯入</h2>
              <FileUp className="text-[var(--amber)]" size={18} />
            </div>
            <p className="mt-3 text-sm leading-6 text-[var(--muted)]">適合人工下載後測試解析品質，資料仍會進入同一個 SQLite 資料庫。</p>
            <input name="title" className="focus-ring mt-4 w-full border border-[var(--line)] bg-white p-2 text-sm" placeholder="公告標題" />
        ...
        <section className="ledger-panel mt-5 p-4">
          <h2 className="font-medium">解析紀錄</h2>
